...
while(True):
    ...
    if pag.url == "https://www.innvictus.com/login/ajax?error=true":
        ...
        search_url = "https://www.innvictus.com/mujeres/basket/tenis/jordan/tenis-air-jordan-4-retro-starfish/p/000000000000188156"
        product_site = scraper.get(search_url,headers=headers)
        if product_site.url == search_url:
            ...
            atc_pag = scraper.post("https://www.innvictus.com/cart/add", atc_data,headers=headers)
            ...
            tiempo_total = time.time() - startTime       #--------------------------------------------------DISCORD-------------------------------------------
            ...
            embed.set_author(name='COP! COP! COP!')
